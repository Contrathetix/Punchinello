...
namespace Punchinello::JsonManager {
	...
	private:
		...
		JsonObject(char *FilenameFromUser) {
			...
			Punchinello::MemoryCache::Find(Filename, Data);
			if (Data.size() > 0) {
			...
			try {
				std::ifstream Filehandle(Punchinello::Interfaces::kOblivionDirectory + Filename);
				Filehandle >> Data;
				Filehandle.close();
				Punchinello::MemoryCache::Push(Filename, Data);
			} catch (std::exception &e) {
